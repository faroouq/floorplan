...
%               by how much each of the covered if the bounding box is 
%               shared equaly amongst them. A box gets 1 when is completely
%               cover its own portion or more.
%
...
% gd1=mean(g1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gmLeft, gmRight]=goodness4portion(W, H, Op)
    ...
    gmLeft=zeros(1,sz);             %initializing variables with the right
    gmRight=zeros(1,sz);            %size improves the performance of MATLAB
    cnt=1;                          %index to the gm
    for L=combo1                    %for all combinations lets find the ratio
        for R=combo2
            gmLeft(cnt)=min((L/(L+R))/0.5, 1);
            gmRight(cnt)=min((R/(L+R))/0.5, 1);
            cnt=cnt+1;
       end
    end
end